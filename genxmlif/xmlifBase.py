...
# 2015-04-08 rl   bugfix in startElementHandler, method getWellknownNamespaces added
#
# Copyright (c) 2005-2015 by Roland Leuthe.  All rights reserved.
#
# --------------------------------------------------------------------
...
class XmlIfBuilderExtensionBase:
    ...
    def startElementHandler (self, curNode, startLineNumber, curNs, attributes=[]):
        ...
        if attributes != []:
            for i in range (0, len(attributes), 2):
                nsNameAttrName = NsNameTupleFactory(attributes[i])
                elementWrapper.attributeSequence.append(nsNameAttrName)
        else:
    ...
    def getWellknownNamespaces (self):
        """Retrieve wellknown namespaces."""
        return [("xml", XML_NAMESPACE), ("xmlns", XMLNS_NAMESPACE)]


    def getSpecificNamespaces (self, namespaces):
        """Retrieve specific namespaces (filter wellknown namespaces)."""
        return filter(lambda x: x not in [("xml", XML_NAMESPACE), ("xmlns", XMLNS_NAMESPACE)], namespaces)
